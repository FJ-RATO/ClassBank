...
static int _treeGetHeightREC(const struct _BSTreeNode* root) {
  ...
  if(root == NULL){
    ...
    int right_branch = -1, left_branch = -1;
    if(root->left != NULL){
      left_branch = _treeGetHeight(root->left);
    }
    if(root->right != NULL){
      right_branch = _treeGetHeight(root->right);
    }
    depth = (right_branch >= left_branch ? right_branch : left_branch) +1;
...
void* BSTreeGetMin(const BSTree* header) {
  ...
  if(aux == NULL){
    perror("BSTreeGetMin: Something went wrong");
  }
  while(aux->left == NULL){
    aux = aux->left;
  }
...
// A: Sim, visto que a fila é criada e ordenada crescentemente
